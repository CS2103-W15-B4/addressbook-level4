...
Usage: Put your mouse on the person you want to use map function on and click(Or use the `select` command to select this person), then enter your current position in starting point to plan a route to this person's home.
...
.  Double-click the file to start the app. You will see 3W's Graphical User Interface (GUI) shown below in a few seconds.
+
...
.  You are now ready to use 3W. Please refer to the link:#features[Features] section below for details of each command.
